...
with open("waveform.csv","w",newline="") as f:
    ...
    # Usiamo il punto e virgola per incolonnare correttamente in Excel italiano
    writer = csv.writer(f, delimiter=';')
    writer.writerow(["time_s", "voltage_CH1_V", "voltage_CH2_V"])

    length = min(len(t1), len(t2))
    for i in range(length):
        # Formattiamo i numeri sostituendo il punto con la virgola decimale
        t_str = str(t1[i]).replace('.', ',')
        v1_str = str(v1[i]).replace('.', ',')
        v2_str = str(v2[i]).replace('.', ',')
        writer.writerow([t_str, v1_str, v2_str])
